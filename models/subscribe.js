...
async function selectSubscribe(uid = 0) {
  try {
    const sql = `SELECT uid,uname,last_title,last_url,date_format(last_feed_time,'%Y-%m-%d %T') last_feed_time ,date_format(last_crawle_time,'%Y-%m-%d %T') last_crawle_time FROM subscribe where deleted = 0 ${uid > 0 ? 'and uid = ?' : ""} order by last_crawle_time;`;
    const values = [uid];
    return await query(sql, values);
  } catch (error) {
    util.log(error.toString(), 'error');
